  ...
  // @ts-expect-error  
    if (!user.isActive) {
      ...
      return null;
                  ...
                  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-expect-error  
    return {id: user._id.toString(),name: user.name,phone: user.phone,email: user.email,role: user.role,isActive: user.isActive,isVerified: user.isVerified,avatar: user.avatar || undefined,};
...
export function validateApiKeyFormat(apiKey: string): boolean {
  ...
  return /^ccg_(live|test)_[a-f0-9]{32}$/.test(apiKey);
}
