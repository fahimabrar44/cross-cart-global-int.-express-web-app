    ...
    apiKey: { 
      ...
      validate: {
        validator: function(v: string) {
          // API key format: ccg_live_32characters or ccg_test_32characters
          return /^ccg_(live|test)_[a-f0-9]{32}$/.test(v);
        },
...
apiConfigSchema.methods.updateUsage = async function (): Promise<void> {
  ...
  const dailyRecord =  this.usage.dailyUsage.find(
  ...
  if (dailyRecord) {
    ...
    if (this.usage.dailyUsage.length > 30) {
      this.usage.dailyUsage = this.usage.dailyUsage// eslint-disable-next-line @typescript-eslint/no-explicit-any
...
apiConfigSchema.methods.generateNewKey = function (): string {
  ...
  this.apiKey = `ccg_${environment}_${randomBytes}`;
  return this.apiKey;
};
