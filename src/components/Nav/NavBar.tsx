...
const NavData = [
  ...
  //       description:
  //         "Discover how CrossCart Global Int Express Courier started and our mission to revolutionize delivery services.",
  ...
  //       description:
  //         "Meet the dedicated professionals driving CrossCart Global Int Express Courier’s success and innovation.",
...
const NavBar = () => {
  ...
  return (
    <header
      className={`z-[40] w-full  bg-white border-b border-soft-green ${
        navBarScrolled
          ? " fixed w-full h-auto animate-in duration-100 shadow-card"
          ...
          <NavigationMenu className="hidden lg:flex">
            <NavigationMenuList className="space-x-1">
              ...
              {/* <NavigationMenuItem className="font-semibold text-black hover:text-black">
                ...
                <NavigationMenuContent className="bg-[#12352A] border-[#12352A] border">
                      ...
                      <NavigationMenuLink asChild>
                        ...
                        >
                          <div className="mb-2 text-lg font-semibold text-white">
                            CrossCart Global Int Express Courier
                          </div>
              ...
              <NavigationMenuItem className="font-semibold text-foreground hover:text-primary">
                <NavigationMenuTrigger className="font-semibold text-foreground hover:text-primary">
                  {NavData?.[1]?.title}
                </NavigationMenuTrigger>
                <NavigationMenuContent className="bg-white border-border border rounded-xl shadow-hover">
                    ...
                    <div className="px-4 w-full absolute bottom-3">
                      {isAuthenticated && (
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild className="h-13 w-full">
                            <Button
                              size="lg"
                              className="bg-white text-foreground hover:bg-white p-2 shadow-card"
                            >
                              <Avatar className="h-8 w-8 rounded-lg">
                                <AvatarImage src={user?.avatar} alt={user?.name} />
                      ...
                      {!isAuthenticated && (
                        <div className="w-full h-auto flex gap-3 justify-center align-middle items-center">
                          <Link
                            href={"/auth/signin"}
                            className="px-3 py-4 flex justify-center align-middle items-center w-[48%] bg-primary hover:bg-[#087F4F] cursor-pointer font-bold text-white rounded-lg transition-colors"
                          >
                            Sign In
                          </Link>
                          <Link
                            href={"/auth/signup"}
                            className="px-3 py-4 text-white w-[48%] bg-primary hover:bg-[#087F4F] rounded-lg cursor-pointer font-bold flex justify-center align-middle items-center transition-colors"
                          >
                            Sign Up
                          </Link>
                        </div>
  ...
  return (
    <li {...props}>
      <NavigationMenuLink asChild>
        <Link
          href={href}
          className="block rounded-lg px-3 py-2 transition text-sm font-medium leading-tight hover:bg-soft-green"
        >
...
const SubMenuLink = ({ item }: { item: MenuItem }) => {
  return (
    <Link
      className=" hover:text-primary flex select-none flex-row gap-4 rounded-lg p-3 leading-none no-underline outline-none hover:bg-soft-green group transition-all duration-250"
      href={item.href}
      onClick={() => item.setMobileMenuOpen?.(false)}
    >
      <div className="text-primary group-hover:text-primary transition-all duration-250">
        {item.icon}
      </div>
      <div>
...
const renderMobileMenuItem = (item: MenuItem) => {
  if (item.items?.length) {
    return (
      ...
      >
        <AccordionTrigger className="text-md py-0 font-semibold hover:no-underline cursor-pointer text-foreground">
          {item.title}
        </AccordionTrigger>
        <AccordionContent
          className="mt-2"
