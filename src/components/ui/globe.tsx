...
const Globe: React.FC<GlobeProps> = ({
  ...
  mapSamples = 60000,
  mapBrightness = 10,
  baseColor = "#087F4F", // CrossCart brand color
  markerColor = "#F5C400", // CrossCart brand yellow
  glowColor = "#087F4F", // CrossCart brand color
  ...
  // WebGL is required by cobe. If the browser does not support WebGL,
  // the globe UI is hidden completely (no crash, no fallback).
  const [webglOk, setWebglOk] = useState(true);

  useEffect(() => {
    // Probe WebGL support before doing anything
    try {
    ...
    const initGlobe = () => {
      ...
      try {
        ...
        // If WebGL is unavailable at init time, hide the globe entirely
        setWebglOk(false);
      }
    };
