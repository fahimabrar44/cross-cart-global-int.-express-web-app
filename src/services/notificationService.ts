...
export class NotificationService {
  ...
  static async sendNotification(
    ...
    try {
      ...
      if (channels.includes("sms") && user.phone && user.preferences?.notifications?.sms !== false) {
        deliveryPromises.push(this.sendSmsNotification({
          phone: user.phone,
  ...
  private static async sendSmsNotification(data: {
    phone: string;
    message: string;
  }): Promise<boolean> {
    try {
  ...
  private static getAuthNotificationTemplate(userData: any, event: string) {
    const templates = {
      welcome: {
        ...
        message: `Hi ${userData.name}! Welcome to CrossCart Global Int Express International Courier.`,
        type: "success" as const,
