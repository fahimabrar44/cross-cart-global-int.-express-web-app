...
export class AuthMiddleware {
  ...
  // @ts-expect-error  
      if (!user || !user.isActive) {
        return null;
                  ...
                  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-expect-error  
      return {id: user._id.toString(),name: user.name,phone: user.phone,email: user.email,role: user.role,isActive: user.isActive,isVerified: user.isVerified,permissions: [], };
    } catch (error) {
  ...
  static generateTokens(
    ...
    const accessToken = jwt.sign(accessPayload, JWT_SECRET, {
      ...
      issuer: "crosscart-api",
      audience: "crosscart-client",
    });
